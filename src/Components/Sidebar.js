...
export default function PersistentDrawerLeft() {
  ...
  React.useEffect(() => {
    if (theme === "Dark") {
      document.body.style.backgroundColor = "rgb(33, 38, 49)"; // Dark theme background
       document.body.style.color = "#ffffff"; // Dark theme text
    } 
     else if (theme === "Light") {
      document.body.style.backgroundColor = "#ffffff"; // Light theme background
        document.body.style.color = "black"; // Light theme text
    }
    else {
      document.body.style.backgroundColor = "rgb(33, 38, 49)"; // Auto theme (neutral)
       document.body.style.color = "#ffffff"; // Auto theme text
    }
  }, [theme]);
  ...
  return (
    ...
    {/* // <Box sx={{ display: "flex" }}> */}
      <CssBaseline />
      <AppBar position="fixed" open={open} openDrawer={handleDrawerOpen} sx={{backgroundColor:'#323a49'}}>
        <Toolbar>
          <IconButton
            color="inherit"
      ...
      >
        <DrawerHeader>
          <Typography variant="h6" sx={{fontWeight:'bold',fontFamily:"Rubik",color:'white'}}>Nagesh Vandana </Typography>
      ...
      {sidebarlist.map((item, index) => (
        <ListItem key={index} disablePadding>
          <ListItemButton onClick={() => navigate(item.rout)}>
            <ListItemText sx={{fontWeight: 700, color:'white', marginLeft:'25px',fontFamily:"Rubik"}} primary= {item.name}
